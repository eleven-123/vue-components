...
export default {
	...
	methods: {
		...
		handleRotate(e) {
			...
			const move = (moveEvent) => {
					...
					const rotateDegreeAfter =
							Math.atan2(curY - centerY, curX - centerX) /
							(Math.PI / 180);
					// 获取旋转的角度值
					pos.rotate =
		...
		handleMouseDownOnShape(e) {
			...
			this.$store.commit("setClickComponentStatus", true);
			if (this.isPreventDrop(this.element.component)) {
					e.preventDefault();
			}
		...
		isNeedLockProportion() {
			...
			for (const component of this.element.propValue) {
					if (
							!ratates.includes(mod360(parseInt(component.style.rotate)))
					) {
							return true;
					}
			}
